...
function ErrorBanner({
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 px-4 py-3 rounded-xl border border-accent-pink/30 bg-accent-pink/10 text-accent-pink text-sm mb-4"
    >
      <TbX size={16} className="shrink-0" />
...
function validate(
  ...
  if (!fields.name.trim()) {
    errors.name = t('contact.form.name_error_required') as string;
  } else if (fields.name.trim().length < 2) {
    errors.name = t('contact.form.name_error_min') as string;
  }

  if (!fields.email.trim()) {
    errors.email = t('contact.form.email_error_required') as string;
  } else if (!EMAIL_RE.test(fields.email)) {
    errors.email = t('contact.form.email_error_invalid') as string;
  }

  if (!fields.message.trim()) {
    errors.message = t('contact.form.message_error_required') as string;
  } else if (fields.message.trim().length < 10) {
    errors.message = t('contact.form.message_error_min') as string;
  }
...
export default function ContactSection() {
  ...
  useEffect(() => {
    if (formStatus === 'error') {
      errorTimer.current = setTimeout(() => setFormStatus('idle'), 5000);
    }
    return () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMessage(
          data?.error ?? (t('contact.form.error_generic') as string)
        );
        setFormStatus('error');
      }
    } catch {
      setErrorMessage(t('contact.form.error_generic') as string);
      setFormStatus('error');
    } finally {
      setIsLoading(false);
    }
  ...
  const inputError: React.CSSProperties = { borderColor: '#f4b8c1' };
              ...
              >
                <GlassCard>
                  <div className="flex flex-col items-center text-center py-8">
                    ...
                    <button
                      onClick={resetForm}
                      className="mt-7 flex items-center gap-2 px-6 py-2.5 rounded-xl border border-accent-gold/40 bg-accent-gold/10 text-accent-gold text-sm font-medium hover:bg-accent-gold/20 transition-colors"
                    >
                      <TbSend size={15} />
                      {t('contact.form.send_another') as string}
                    </button>
                  </div>
                </GlassCard>
              </motion.div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <input
                          ...
                          disabled={isLoading}
                          placeholder={t('contact.form.name_placeholder') as string}
                          style={{
                            ...inputBase,
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <input
                          ...
                          onChange={handleChange}
                          disabled={isLoading}
                          placeholder={t('contact.form.email_placeholder') as string}
                          style={{
                            ...inputBase,
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <textarea
                        ...
                        rows={5}
                        placeholder={
                          t('contact.form.message_placeholder') as string
                        }
                        style={{
                          ...inputBase,
                          resize: 'none',
                    ...
                    <div className="flex justify-end">
                      <button
                        type="submit"
                        disabled={isLoading}
                        className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-accent-gold text-bg font-semibold text-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed hover:brightness-110 active:scale-95"
                      >
                        {isLoading ? (
                          <>
